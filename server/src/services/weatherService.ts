...
export async function generateAiBriefing(weatherData: any): Promise<string> {
  ...
  try {
    ...
    const prompt = `You are a helpful weather assistant. Summarize the following weather forecast into a single short paragraph (max 20-30 words, similar to: "Conditions will remain mild throughout the day, with rain possible during the afternoon. Plan outdoor activities for earlier hours where possible.").
...
Upcoming 24-Hour Forecast Highlights:
${hourlyHighlights}

Daily Outlook for next few days:
${dailyOutlook}`;
